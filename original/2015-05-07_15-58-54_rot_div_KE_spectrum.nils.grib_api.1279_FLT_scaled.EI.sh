...
cat >dir<< EOF
retrieve,date=$DATE,class=$CLASS,expver="$EXP",levtype=pl,type=an,
   level=$LEV,param=D/VO,step=0,time=$TIME,target="targx1"
EOF
mars dir
...
#mars dir2
./spect
scp outx2 cormac:/scratch/rd/naw/curv.${LEV}.${EXP}.${STEP}.INI.rot.div.dat

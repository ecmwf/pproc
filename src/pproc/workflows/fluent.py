...
class Action(fluent.Action):
    ...
    def thermal_index(
        ...
        thermal_functions = {
            "utci": self.thermal_indices.utci,
            "mrt": self.thermal_indices.mrt,
            "heatx": self.thermal_indices.heatx,
            "wcf": self.thermal_indices.wcf,
            "aptmp": self.thermal_indices.aptmp,
            "2r": self.thermal_indices.rhp,
            "hmdx": self.thermal_indices.hmdx,
            "nefft": self.thermal_indices.nefft,
            "gt": self.thermal_indices.gt,
            "wbgt": self.thermal_indices.wbgt,            
        }
        return thermal_functions[param](dim=dim, metadata=metadata)
...
RequestList = list[dict | Request | MultiSourceRequest]

def from_source(
    requests: RequestList | dict[str, RequestList],
    join_key: str = "",
    backend_kwargs: dict = {},
):
    all_actions = None
    if not insinstance(requests, dict):
        requests = {"/": requests}

    for path, prequests in requests.items():
        path_action = None
        for request in prequests:
            ...
            if path_action is None:
                path_action = new_action
            else:
                if len(join_key) == 0:
                    raise ValueError("Join key must be specified for multiple requests")
                path_action = path_action.join(new_action, join_key)
        if all_actions is None:
            all_actions = path_action
        else:
            all_actions = all_actions.merge(path_action)
    return all_actions
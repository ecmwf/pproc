...
def test_config_uniqueness(temp_path):
    ...
    recover1 = Recovery(temp_path, f"{temp_path}/config1.yaml", datetime.datetime(2023, 1, 1), recover=False)
    recover2 = Recovery(temp_path, f"{temp_path}/config2.yaml", datetime.datetime(2023, 1, 1), recover=False)
    assert recover1.filename != recover2.filename 


@pytest.mark.parametrize("recover, num_checkpoints", [
    (True, 1), (False, 0)])
def test_recovery(temp_path, recover: bool, num_checkpoints: int):
    ...
    recover2 = Recovery(temp_path, f"{temp_path}/config1.yaml", datetime.datetime(2023, 1, 1), recover=recover)

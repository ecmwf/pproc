...
class TropicalCyclone:
    ...
    def distance_from(self, tc_other):
        return self._haversine_dist(self.lat, self.lon, tc_other.lat, tc_other.lon)
...
class TRCKR_Forecast(TC_Forecast):
    ...
    required_fields = ["name", "step", "lat_p", "lon_p"]
    ...
    def _parse(self):
        ...
        for step in self.search_steps:
            tc_predictions[step] = [
                TropicalCyclone(name, lat, lon, wind=w, p=p, step=step)
...
class TCYCL_Forecast(TRCKR_Forecast):
    ...
    required_fields = ["name", "step", "lat", "lon"]
    ...
    def __init__(self, reference_tc_path, min_wind_speed=None, search_steps=None):
        super().__init__(
            reference_tc_path, min_wind_speed=min_wind_speed, search_steps=search_steps
        )
...
class IBTKS_Data(TC_Observation):
    ...
    def __init__(self, reference_tc_path, min_wind_speed=None):
        self.df = pd.read_csv(reference_tc_path)
    ...
    def _parse(self):
        # Collect TC tracker prediction data
        tc_data = [
            TropicalCyclone(name, lat, lon, wind=w, p=p)
...
class ConfusionMatrix(Scorer):
    ...
    def __init__(self, ref_data, tcycl_data, threshold, compare_at_steps=None):
    ...
    def _print_score(self):
        if self._score is None:
            print("Score cannot be printed. Invoke Run first.")
            raise ValueError
    ...
    def run(self):
        ...
        for step in self.compare_at_steps:
            ...
            print(f"#### Step {step}:")
            print(
                f"--> TRACK-cyclones ({len(s_trckr)}):",
                *[str(t) for t in s_trckr],
                sep="\n",
            )
            print(
                f"--> TCYCL-cyclones ({len(s_tcycl)}):",
                *[str(t) for t in s_tcycl],
                sep="\n",
            )
...
class TCPlotter(Scorer):
    ...
    def __init__(
        self, trckr_pred, tcycl_pred, plot_steps_str, output_dir, ibtracks=None
    ):
    ...
    def run(self):
        ...
        for step in self.plot_steps:
            ...
            trckr_style = {
                "c": [tc.params["p"] for tc in self.trckr_prediction[step]],
                "cmap": trckr_cmap,
                "s": int(marker_size * 2),
                "marker": "o",
                "vmin": p_min,
            ...
            tcycl_style = {"c": "k", "s": marker_size, "marker": "x"}
            ...
            # background map
            bk_cmap = plt_cols.ListedColormap(
                [[0.3, 0.4, 0.2, 0.8], [0.7, 0.7, 0.9, 0.9]]
            )
            plt.imshow(
                np.load(os.path.join(this_dir, "map.npy")),
                extent=[0, 360, -90, 90],
                cmap=bk_cmap,
            )
            ...
            tcycl_lons = [
                coords.lon if coords.lon >= 0 else coords.lon + 360
                for coords in self.tcycl_prediction[step]
            ]

            im_trckr = plt.scatter(
                trckr_lons, trckr_lats, label="TC-tracker", **trckr_style
            )
            ...
            if self.ibtracks:
                ...
                _ = plt.scatter(
                    ibtks_lons, ibtks_lats, label="IB-Tracks", **ibtks_style
                )
...
def main():
    ...
    parser.add_argument("trckr_summary_path", help="Path to CSV with Tracker summary")
    parser.add_argument("tcycl_summary_path", help="Path to CSV with Tcycl summary")

    parser.add_argument(
        "--dist_threshold", help="TC distance threshold [Km]", type=int, default=300
    )

    parser.add_argument(
        "--trckr_w_min", help="TC-Tracker TC's with wind speed >= threshold", type=float
    )
    parser.add_argument(
        "--tcycl_w_min", help="TC-Tracker TC's with wind speed >= threshold", type=float
    )

    parser.add_argument(
        "--plot_steps",
        help="Make plots for steps (comma-separated list - e.g. '6,12,18')",
    )
    parser.add_argument(
        "--output_dir", help="Directory where plots are saved", default="/var/tmp/plots"
    )

    parser.add_argument("--ibtracks_summary_path", help="Path to IBTracks CSV summary")

    args = parser.parse_args()

    # # TC tracker and TCYCL data
    trckr_prediction = TRCKR_Forecast(
        args.trckr_summary_path, min_wind_speed=args.trckr_w_min
    )
    tcycl_prediction = TCYCL_Forecast(
        args.tcycl_summary_path, min_wind_speed=args.tcycl_w_min
    )
    ...
    if args.ibtracks_summary_path:
        ibtks_data = IBTKS_Data(
            args.ibtracks_summary_path, min_wind_speed=args.tcycl_w_min
        )

...
def calc_dist(latdiff, londiff, lat):
    ...
    a = np.sin(dlat / 2) * np.sin(dlat / 2) + np.cos(np.radians(lat)) * np.cos(
        np.radians(lat + latdiff)
...
def calc_speed3(my_pd):
    ...
    speedcum = dist.cumsum() / 1000.0  # km
...
def read_oper_fc(filename, date1, basin, Pall, obsfile="none"):
    ...
    assert obsfile == "none", (
        "Error, for tcycl validation purposes " "obsfile must be set to 'none'"
    )
    ...
    if obsfile != "none":
        read_besttrack_date(date1, obsfile)
    ...
    while line:
        ...
        if s1[1][0:4] == recon:
            ...
            day = line[14:16]
            time = line[17:19]
            lat1 = lat_mult * float(line[20:23]) / 10.0 + 0.05
            lon1 = float(line[23:27]) / 10.0 + 0.05
            wind = float(line[28:31])
            pres = float(line[32:36])
            lat2 = lat_mult * float(line[37:40]) / 10.0 + 0.05
            lon2 = float(line[40:44]) / 10.0 + 0.05
            ...
            df1["step"] = (
                (df1["datetime"] - date1).values.astype("timedelta64[h]").astype(int)
            )
            df1["windrad"] = (
                calc_dist(df1.lat_p - df1.lat_w, df1.lon_p - df1.lon_w, df1.lat_p)
                / 1000.0
            )

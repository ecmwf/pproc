...
class Utils:
    ...
    def ifs_steps(max_step):
        ...
        return (
            list(range(1, min(90, max_step + 1), 1))
...
class TCYCLConfig(JSONConfigurable):
    ...
    _default_config = {
        ...
        "cyclone_localizer": {"label_threshold": 0.6, "label_radius": 25},
    }
...
class FieldsData:
    ...
    def get_fields(self, **kwargs):
        ...
        for k in kwargs.keys():
            assert (
                k in FieldData.required_keys
            ), f"Searching field by non-existent param {k}"
...
class DataRetriever(JSONConfigurable):
    ...
    _required_config_params = ["variables", "grid_resolution", "n_prev_steps"]
...
class FDBDataRetriever(DataRetriever):
    ...
    _req_template_hres = {"stream": "oper", "type": "fc"}

    _req_template_ens_pf = {"number": None, "stream": "enfo", "type": "pf"}

    _req_template_ens_cf = {"stream": "enfo", "type": "cf"}

    # required keys in GRIB message
    required_grib_keys = ["shortName", "Ni", "Nj", "dataDate", "dataTime", "stepRange"]

    # required keys in user request
    _required_user_req_params = ["date", "time", "ens", "step"]
    ...
    def __init__(self, config):
        ...
        self._req_template_base["param"] = [
            self.p_map.get(p) for p in config["variables"] if self.p_map.get(p)
        ]
    ...
    def _compose_fdb_request(self, user_request):
        ...
        assert -1 <= user_request["ens"] <= 51, (
            "Request ens must be in range [-1,50]. " " Legend (-1:HRES, 0:cf, 1,50:pf)!"
        )
        ...
        assert int(user_request["step"]) >= 3, (
            f"Min step is 3." f"Requested step {user_request['step']}!"
        )
...
data_retrievers = {"fdb": FDBDataRetriever}
...
class PreProcessor(JSONConfigurable):
    ...
    def run(self, input_data):
        ...
        for mm, field_data in enumerate(processed_data.fields):
            ...
            processed_data.fields[mm].values = (
                field_data.values - shift_factor
            ) / norm_factor
...
class Inference(JSONConfigurable):
    ...
    _required_config_params = ["model_path", "model_type", "model_output_shape"]
...
class CycloneLocalizer_DBSCAN(JSONConfigurable):
    ...
    _required_config_params = ["label_threshold", "label_radius"]
...
class TCAnnotator(JSONConfigurable):
    ...
    def run(self, predictions, data, save_to=None):
        ...
        for pred in predictions_pxl:
            ...
            diff = mesh_zipped - np.tile(
                p, (*data.metadata["field_shape"], 1)
            )  # coord diff
            ...
            print(
                f"v_max {vabs_max:.2f} (in lat:{vabs_max_lat:.2f}, lon:{vabs_max_lon:.2f})"
            )

            tc_annotations.append(
                [p_min, p_min_lat, p_min_lon, vabs_max, vabs_max_lat, vabs_max_lon]
            )
...
def main():
    ...
    parser.add_argument("json_request", help="Path to JSON data request")

    parser.add_argument("--config", help="JSON of tcycl user-configuration")
    parser.add_argument("--save_ml_output", help="Path to numpy ML prediction file")
    parser.add_argument("--save_prediction", help="Path to JSON prediction file")
    parser.add_argument("--annotate_prediction", help="Path to JSON annotation file")
    ...
    prediction = localizer.find(
        output_data, pre_proc_data.metadata, save_to=args.save_prediction
    )

    # Annotate prediction
    if args.annotate_prediction:
        annotation_data_retriever = data_retrievers[config.data_fetching["retriever"]](
            config.annotations
        )
        annotation_data = annotation_data_retriever.get_data_from_json_request(
            args.json_request
        )
        annotator = TCAnnotator(config.annotations)
        annotator.run(prediction, annotation_data, save_to=args.annotate_prediction)

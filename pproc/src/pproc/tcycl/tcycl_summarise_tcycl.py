...
class SummaryWriter:
    ...
    def __init__(self, prediction_files, annotation_files=None):
        ...
        if annotation_files is not None:
            ...
            annotations_files_dict = {
                int(anno_name.split(".")[1].split("step")[1]): anno_name
...
def main():
    ...
    parser.add_argument(
        "prediction_files",
        help=";-separated list of prediction files (as prediction.step<N>.json)",
    )
    parser.add_argument(
        "--annotation_files",
        help=";-separated list of annotation files (as annotation.step<N>.json)",
    )
    parser.add_argument(
        "--output", help="Path to output CSV file", default="output.csv"
    )
    args = parser.parse_args()

    # Make summary file
    writer = SummaryWriter(
        args.prediction_files, annotation_files=args.annotation_files
    )

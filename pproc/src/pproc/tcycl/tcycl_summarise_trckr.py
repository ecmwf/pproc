...
class Experiment:
    ...
    basin_list = ["wnp", "spc", "sin", "nin", "enp", "cnp", "aus", "atl"]
    ...
    def _write_csv(self, date_from, date_to):
        ...
        csv_summary_path = os.path.join(
            self.out_dir, f"tc_tracker_data_{self.name}.csv"
        )
...
def main():
    ...
    parser.add_argument(
        "date_from",
        help="Requested data from (format 'YYYYMMDD TT', e.g. '20211004 00')",
    )
    parser.add_argument(
        "date_to", help="Requested data to (format 'YYYYMMDD TT', e.g. '20211004 12')"
    )
    parser.add_argument(
        "--output_dir", help="Output directory", default="/var/tmp/tc_tracker_data"
    )
    parser.add_argument(
        "--data_types", help="Data types requested (e.g. hres,pf,cf)", default="hres"
    )
